...
export const sendOrderConfirmation = async ({ buyerName, buyerEmail, buyerPhone, productName, amount, orderId, isDigital, downloadLink }) => {
  try {
    await emailjs.send(SERVICE_ID, ORDER_CONFIRM_TEMPLATE, {
      buyer_name: buyerName, buyer_email: buyerEmail, buyer_phone: buyerPhone || 'N/A',
      product_name: productName, amount: amount.toLocaleString(), order_id: orderId,
      is_digital: isDigital ? 'true' : '', is_physical: !isDigital ? 'true' : '', download_link: downloadLink || '',
    }, PUBLIC_KEY)
    return { success: true }
...
export const sendNewOrderToSeller = async ({ sellerName, sellerEmail, buyerEmail, buyerPhone, productName, amount, orderId, isDigital, deliveryAddress }) => {
  try {
    await emailjs.send(SERVICE_ID, NEW_ORDER_TEMPLATE, {
      seller_name: sellerName, seller_email: sellerEmail, buyer_email: buyerEmail,
      buyer_phone: buyerPhone || 'N/A', product_name: productName, amount: amount.toLocaleString(),
      order_id: orderId, is_digital: isDigital ? 'true' : '', is_physical: !isDigital ? 'true' : '',
