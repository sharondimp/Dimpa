  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const pQ = query(collection(db, 'products'), where('sellerId', '==', user.uid), where('status', '==', 'active'))
        const pSnap = await getDocs(pQ)
        setProducts(pSnap.docs.map(d => ({ id: d.id, ...d.data() })))

        // Get seller's sponsored submissions
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg2)' }}>
      ...
      <div style={{ paddingTop: '64px', padding: '5rem 5% 3rem' }}>
        ...
        <div className="card" style={{ marginBottom: '2rem', background: 'var(--green-soft)', border: '1px solid rgba(0,168,120,0.2)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '0.75rem' }}>
            {[
              ...
              ['2️⃣', `Pay ₦${price.toLocaleString()} (${isPremium ? '50% off — Premium!' : '₦1,000'})`],
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: '1.5rem' }}>
          ...
          <div className="card" style={{ padding: '1.8rem' }}>
            ...
            <div className="form-group">
              ...
              {products.length === 0 ? (
                <div style={{ fontSize: '0.85rem', color: 'var(--muted)', padding: '0.7rem', background: 'var(--bg3)', borderRadius: '8px' }}>No active products yet. Add a product first.</div>
              ) : (
            ...
            <div className="form-group">
              ...
              {slotsInfo.date === form.date && (
                <span className={`form-hint`} style={{ color: slotsInfo.available <= 0 ? 'var(--danger)' : 'var(--green)' }}>
            ...
            <button onClick={handlePay} className="btn-primary" style={{ width: '100%', justifyContent: 'center', padding: '0.8rem' }} disabled={paying || products.length === 0}>
              {paying ? 'Processing...' : `Pay ₦${price.toLocaleString()} via Paystack`}
            </button>
        ...
        <style>{`@media (max-width: 640px) { div[style*="gridTemplateColumns: '1fr 320px'"] { grid-template-columns: 1fr !important; } }`}</style>
      </div>
    </div>
  )
}
