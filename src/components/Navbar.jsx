  ...
  return (
    <>
      <header style={{
        ...
        background: theme === 'dark' ? 'rgba(8,11,30,0.95)' : 'rgba(255,255,255,0.95)',
        ...
        <button onClick={() => setMenuOpen(!menuOpen)} className="mob-menu-btn" style={{ flexDirection: 'column', gap: '5px', padding: '6px', background: 'none', border: 'none', cursor: 'pointer', display: 'none' }}>
      ...
      {menuOpen && (
        <div style={{ position: 'fixed', top: '64px', left: 0, right: 0, zIndex: 999, background: 'var(--bg)', borderBottom: '1px solid var(--border)', padding: '1rem 5% 1.5rem', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 30px rgba(0,0,0,0.15)' }}>
          {variant === 'public' && <>
          ...
          {variant === 'dashboard' && <>
            ...
            <Link to="/dashboard/upgrade" onClick={() => setMenuOpen(false)} style={{ padding: '0.75rem 0', borderBottom: '1px solid var(--border)', color: 'var(--blue)', fontWeight: 600, textDecoration: 'none' }}>⚡ Upgrade</Link>
          </>}
          {variant === 'admin' && <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.75rem 0', marginTop: '0.5rem', borderTop: '1px solid var(--border)' }}>
            ...
            <div onClick={toggleTheme} style={{ width: '44px', height: '24px', borderRadius: '50px', cursor: 'pointer', background: theme === 'dark' ? 'var(--blue)' : 'var(--bg3)', position: 'relative', transition: 'background 0.3s', flexShrink: 0 }}>
